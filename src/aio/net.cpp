...
TcpServer::TcpServer(size_t port) {
  ...
  set_fd_nonblock(this->_fd);
  ::bind(this->_fd, (::sockaddr*)(&this->_svr_addr), sizeof(this->_svr_addr));
  ::listen(this->_fd, ListenCapacity);
 ...
 public:
  ...
  auto send(const std::string& data) -> Async<void> {
    for (int i = 0; i < data.size(); i += BufferSize) {
      ...
      DEBUG("copnnection fd=%lu send %d bytes", this->_fd, nbytes);
      if (nbytes <= 0) {
        // TODO: error handle if nbytes==-1

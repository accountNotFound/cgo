...
void Context::start(size_t worker_num) {
  _impl::_sched::g_enginee = std::make_unique<_impl::_sched::Enginee>(worker_num);
...
void Context::stop() {
  ...
  _impl::_sched::g_enginee = nullptr;
}

void Context::run_worker(size_t index) {
  while (!this->_finished) {
    if (_impl::_sched::g_enginee->execute(index)) {
      continue;
    }
...
void start_context(size_t worker_num) {
  ...
  _impl::_ctx::g_context->start(worker_num);
}

void stop_context() {
  _impl::_ctx::g_context->stop();
